...
const NewProduct = () => {
  ...
  const handleFlashSaleChange = (e) => {
    ...
    console.log('Flash Sale State:', e.target.checked); // Debugging the state
  };
  ...
  const handleNewProduct = async() =>{
    await axios.post('http://localhost:6001/add-new-product', {productName, productDescription, productMainImg, productCarousel: [productCarouselImg1, productCarouselImg2, productCarouselImg3], productSizes, productGender, productCategory, productNewCategory, productPrice, productDiscount,isFlashSale}).then(
      (response)=>{
        alert("product added");
        setProductName('');
  ...
  return (
    ...
    />
    <label className="form-check-label" htmlFor="flashSaleCheckbox" style={{ color: 'white' }}>
      Flash Sale
    </label>
  </div>
</section>
